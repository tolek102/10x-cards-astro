...
#### 2.2. Niespójne użycie serwisów klienckich
...
#### 2.3. Hydratacja komponentu `Logo`
...
#### 2.5. Logika `useFlashcards` dla ręcznie tworzonych fiszek
...
#### 3.2. Logika listowania fiszek w `FlashcardService` (serwer)
...
#### 4.5. `useTheme` z `next-themes` w `ui/sonner.tsx`
...
#### 4.7. Obsługa błędów w `AuthProvider` przy parsowaniu JSON
